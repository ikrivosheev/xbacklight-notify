...
backlight_new(backlight_t* backlight)
{
    ...
    if (_randr_version(backlight->connection, &backlight->version, &error) == FALSE) {
...
_randr_version(xcb_connection_t* connection, version_t* version, xcb_generic_error_t** error)
{
    ...
    cookie = xcb_randr_query_version(connection, 1, 2);
    reply = xcb_randr_query_version_reply(connection, cookie, error);
    if (*error != NULL || reply == NULL) {
        return FALSE;
    }
    version->major = reply->major_version;
    version->minor = reply->minor_version;
    free(reply);
    return TRUE;
}
...
static gboolean
backlight_get_range(xcb_connection_t* connection,
...
static gboolean
backlight_get_value(xcb_connection_t* connection,
                    xcb_randr_output_t output,
                    xcb_atom_t backlight_new,
                    xcb_atom_t backlight_old,
                    long* value,
